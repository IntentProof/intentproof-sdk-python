...
class IntentProofExecutionEventV1(BaseModel):
    ...
    error: ExecutionError | None = Field(
        None, description='Structured error when status is error.'
    )
    ...
    attributes: (
        dict[constr(pattern=r'^[A-Za-z0-9_.:-]{1,256}$'), str | float | bool | None]
        | None
    ) = Field(
        None, description='Flat primitive key/value metadata attached to the event.'
    )
